...
def main(config_path):
    if os.path.isfile('output.txt'):
        print('output.txt already exists. Remove? (y/n)')
        while True:
            user_input = input()
            if user_input == 'y':
    ...
    new_selection_function = eppsea_base.EppseaSelectionFunction(eppsea.population[0])
    ...
    basic_median_best_fitnesses = dict()
    for f in fitness_functions:
        f_results = list(r for r in basic_cmaess_results if r.fitness_function_id == f.id)
        basic_median_best_fitnesses[f.id] = statistics.median(r.final_best_fitness for r in f_results)
    ...
    for f in fitness_functions:
        ...
        for s in selection_functions:
            ...
            percentage_better = len(list(r for r in s_results if r.final_best_fitness <= basic_median_best_fitnesses[f.id])) / len(s_results)
    ...
    for i in (3,5,10,20,50,100, 200, 500, 1000, 2000):
        if i < cmaes.testing_runs:
            ...
            for f in fitness_functions:
                ...
                basic_median_best_fitnesses[f.id] = statistics.mean(r.final_best_fitness for r in f_results)
